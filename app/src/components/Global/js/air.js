( function( window, document, undefined ) {
	...
	var framework = new function() {
		...
		self.ModuleManager = new function() {
			 ...
			 * TODO: do the same for simple functions.
			 * Нельзя вызывать класс без положенных ему аргументов.
			 * @param {Object} module - Module.
			 * @return {Object}.
			 */
			var classGenerator = function( module ) {
				if ( module.parameters.immediately_invoked === true ) {
					return module.constructed = module.content.apply( null, module.deps_kit.concat( makeUtil() ) );
				} else {
					return function() {
			...
			manager.define = function( name, deps_list, content, parameters ) {
					...
					switch ( content_actual_type ) {
						case 'function':
							switch ( content_defined_type ) {
								//стандартный модуль, работающий оп принципу init-refresf-destroy (IRD?)
								case undefined:
								case 'constructor':
				...
				if ( error === undefined ) {
					...
					modules[ modules_length++ ] = {
						uid: self.utility.uid( name ),
						name: name,
						deps_list: self.utility.unique( deps_list ),
						content: content,
						type: content_type,
						parameters: parameters,
						weight: 0
			 ...
			 * Uses "defineWithType" method.
			 */
			manager.defineFn = function( name, content, parameters ) {
				return manager.defineWithType( 'function', name, [], content, parameters );
			};
			 ...
			 * Uses "defineWithType" method.
			 */
			manager.defineLib = function( name, content, parameters ) {
				return manager.defineWithType( 'object', name, [], content, parameters );
			};
					...
					switch ( module.parameters.deps_format ) {
						case 'object':
							...
							deps_list.forEach( function( dep_name ) {
								deps_kit[ dep_name ] = manager.inherit( dep_name );
							} );
							break;
			...
			var arrangeWeights = function() {
				...
				while ( is_weight_changed === true && is_looped === false ) {
					...
					for ( i = 0; i < modules_length; i++ ) {
						...
						for ( j = 0; j < modules_length; j++ ) {
							dep = modules[ j ];
							dep_name = dep.name;
							dep_weight = dep.weight;

							if ( module_deps_list.indexOf( dep_name ) > -1 && module_name !== dep_name && module.weight <= dep_weight ) {
								if ( dep_weight + 1 <= limit_weight ) {
									module.weight = dep_weight + 1;
				...
				if ( is_looped === false ) {
					...
					self.utility.error( 'Module "%s" is in loop', looped_module_name );
				}

				module = null;
				module_deps_list = null;
				dep = null;
			...
			manager.getDOMModules = function() {
				...
				for ( i = 0; i < length; i++ ) {
					...
					if ( module_names_length > 0 ) {
						settings = settings_element ? settings_element.innerHTML.trim() || '{}' : '{}';
						// _log(settings_element,settings);
						// _log(settings_element, ( settings_element && settings_element.innerHTML ), ( settings_element && settings_element.innerHTML ) || '{}');
...
// 		var framework = this;
//
// 		framework.module_manager = new function() {
// 			var manager = this;
//
// 			manager.modules = [];
// 			manager.modules_length = 0;
//
// 			manager.isComponentExists = function( name, element ) {
// 				var i,
// 					is_exists = false;
//
// 				for ( i = 0; i < manager.modules_length; i++ ) {
// 					if ( manager.modules[ i ].type === 'component' && manager.modules[ i ].name === name && manager.modules[ i ].element === element) {
// 						is_exists = true;
// 						break;
// 					}
// 				}
...
// 			};
//
// 			manager.addComponentsFromDocument = function() {
// 				var components = [],
// 					components_length = 0,
// 					elements = document.querySelectorAll( 'air' ),
...
// 					element_parent;
//
// 				for ( i = 0; i < framework_elements_length; i++ ) {
// 					component_name = elements[ i ].getAttribute( 'component' );
//
// 					if ( component_name ) {
// 						component_element = elements[ i ].parentElement;
//
// 						if ( !isComponentExists( component_name, component_element ) ) {
// 							element_depth = 0;
// 							element_parent = component_element;
//
...
// 			};
//
// 			manager.processComponents = function( components ) {
// 				var element_parent,
// 					is_parent_found,
// 					i,
...
// 					element_parent = components[ i ].element.parentElement;
// 					is_parent_found = false;
//
// 					while ( element_parent ) {
// 						for ( j = 0; j < components_length; j++ ) {
// 							if ( i !== j && element_parent === components[ j ].element ) {
// 								components[ i ].parent = components[ j ];
// 								components[ j ].child.push( components[ i ] );
//
